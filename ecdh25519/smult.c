...
/* Alternative: Non-windowed constant-time implementation (backup) */
int crypto_scalarmult_simple(unsigned char *ss, const unsigned char *sk, const unsigned char *pk)
{
  group_ge p, k, temp;
  unsigned char t[32];
  int i, j;
  ...
  /* Constant-time double-and-add from MSB to LSB */
  for(i=255; i>=0; i--)
  {
    group_ge_double(&k, &k);
